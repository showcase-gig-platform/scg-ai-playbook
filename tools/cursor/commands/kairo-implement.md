# Name: kairo-implement

---

# Description: 分割されたタスクを順番に、またはユーザが指定したタスクを実装します。TDDコマンドを活用して品質の高い実装を行います。
...
5. **実装プロセスの実行**
   ...
   a. **要件定義** (`/tdd-requirements`)
   - タスクの詳細要件を記述
   - 受け入れ基準を明確化

   b. **テストケース作成** (`/tdd-testcases`)
   - 単体テストケースを作成
   - エッジケースを考慮
   ...
   e. **リファクタリング** (`/tdd-refactor`)
   - コードの品質向上
   - 保守性の改善

   f. **品質確認** (`/tdd-verify-complete`)
   - 実装の完成度を確認
   - 不足があれば c-f を繰り返す
