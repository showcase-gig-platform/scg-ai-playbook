...
要件定義書を作成する際、プロジェクトの言語に応じて以下を調整してください：

| 言語 | 確認ファイル | 調整内容 |
|------|-------------|---------|
| Go | `go.mod` | Go特有の慣習（エラーハンドリング等）を考慮 |
| Rust | `Cargo.toml` | Result/Option型、所有権を考慮 |
| Python | `pyproject.toml` | 型ヒント、例外処理を考慮 |
| TypeScript | `package.json` | 型システム、非同期処理を考慮 |
| Java | `pom.xml` | 例外階層、インターフェースを考慮 |
...
要件定義が完了したら、`/kairo-design` で技術設計に進んでください。
