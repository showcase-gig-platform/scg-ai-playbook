...
作業開始時、Gitの状態を確認する前にユーザーに以下の情報を質問する。ユーザーに質問をするためのツールが使える場合は必ずそれを使用して質問する。

- ベースブランチ: PRのマージ先ブランチ
  - 選択肢の候補を提示する場合、規定値として `main` を提示する。
  - `git branch` コマンドを実行してブランチ一覧を取得し、もしもその中にベースブランチの候補がある場合はそれも提示する。
- チケットIDまたは関連リンク
