...
3. **未コミットの変更がある場合**
   ...
   - create-commit の手順（変更把握・グループ化）: `git status` / `git diff` / `git diff --cached` で変更を把握し、変更を意味的なグループに分類する（複数目的が混在する場合は複数コミットに分ける想定）。
   - **安全ガード**: 次のいずれかに該当する場合は、自動で `git add` / `git commit` を行わず、**ユーザー確認を必須**とする。
     - **複数意図の変更検出**: グループ化の結果、複数グループ（2 以上）が生成された場合。
     - **機微ファイルの検出**: 変更に含まれるパス・ファイル名が、`secret`、`.env`、`credentials`、`key`、`password`、`token`、`.pem`、`.key` など機微とみなせるパターンにマッチする場合。
   - 該当する場合の確認プロンプトでは、**どのファイル・どのグループが該当したか**を明示する。続行するときは、create-commit のコミットメッセージフォーマット（prefix: 概要、why:、what:）で各コミット案を提示し、ユーザーの承認を得たうえで `git add` / `git commit` を実行する。
   - 上記のいずれにも該当しない場合（単一グループかつ機微ファイルなし）は、確認を取らずにグループごとにステージ・メッセージ生成・`git commit` を実行してよい。コミットメッセージは create-commit のフォーマットに従う。ユーザーにコミットメッセージの確認を取る必要はない（依頼があれば確認する）。
...
3. 本文を生成する。
   ...
   - PR は、完成した差分を初めて見るレビュワーが短時間で変更の目的・結果・影響を判断できる内容にする。会話内の検討、修正、却下した案などの経緯は持ち込まず、現在の差分だけを説明する。
   - PRタイトルは Conventional Commits 形式で、変更の目的または利用者から見た結果が一読で分かる短い文にする。`type`、`scope`、`summary` は差分とコミット履歴から決め、単一の `scope` に絞れない場合は `type: summary` を使う。ファイル名や関数名などの実装詳細は、それ自体が変更対象である場合だけ含める。
   - 各項目は短い段落または数個の箇条書きで、レビュー判断に必要な情報だけを残す。コードを読めば分かる実装詳細、ファイル一覧、コミットごとの説明、変更中の試行錯誤、採用しなかった案は省く。テンプレート上必須だが該当しない項目は「なし」とする。
   - ロジックや処理フローの変更が文章より図で明確になる場合は、GitHub が描画できる `mermaid` コードブロックを使ってよい。単純な変更には図を追加しない。
